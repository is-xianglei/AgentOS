"""
FastAPI SSE 流式后端 — 基于 main.py 逻辑

运行: uv run uvicorn main_sse:app --reload --port 8000
前端: 用浏览器直接打开 main_sse.html（任意目录均可）
"""
import json
import os
...
@app.get("/stream")
async def stream_sse(prompt: str = "你好！先介绍一下自己，然后帮我在当前目录创建 test.txt，再查询北京天气预报"):
    async def generate():
        ...
        while True:
            ...
            for tu in tool_uses:
                ...
                yield f"data: {json.dumps(custom, ensure_ascii=False)}\n\n"
